...
description: >
  Technical guide for creating a new FideliOS agent adapter. Use when building
...
# Creating a FideliOS Agent Adapter

An adapter bridges FideliOS's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `fidelios run --watch`
  package.json
  tsconfig.json
```
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell FideliOS to forget the stored session
}
...
The `agentConfigurationDoc` is read by LLM agents (including FideliOS agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
| Variable | Source |
|----------|--------|
| `FIDELIOS_AGENT_ID` | `agent.id` |
| `FIDELIOS_COMPANY_ID` | `agent.companyId` |
| `FIDELIOS_API_URL` | Server's own URL |
| `FIDELIOS_RUN_ID` | Current run id |
| `FIDELIOS_TASK_ID` | `context.taskId` or `context.issueId` |
| `FIDELIOS_WAKE_REASON` | `context.wakeReason` |
| `FIDELIOS_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `FIDELIOS_APPROVAL_ID` | `context.approvalId` |
| `FIDELIOS_APPROVAL_STATUS` | `context.approvalStatus` |
| `FIDELIOS_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `FIDELIOS_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `fidelios run --watch`. Use `picocolors` for coloring.
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use FideliOS API
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so FideliOS wipes the stale session
...
| `buildFideliOSEnv(agent)` | Standard `FIDELIOS_*` env vars |
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your FideliOS work."`
...
### FideliOS Skills Injection

FideliOS ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `fidelios` API skill and the `fidelios-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with FideliOS internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("fidelios-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  ...
  const entries = await fs.readdir(FIDELIOS_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(FIDELIOS_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks FideliOS skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The FideliOS skills (like `fidelios` and `fidelios-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the FideliOS API to report status), use explicit instructions in the prompt template: "Use the fidelios skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between FideliOS's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `FIDELIOS_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the FideliOS API and GitHub should not have access to arbitrary hosts.
