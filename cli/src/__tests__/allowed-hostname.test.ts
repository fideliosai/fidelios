...
function createTempConfigPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "fidelios-allowed-hostname-"));
  return path.join(dir, "config.json");
}

function writeBaseConfig(configPath: string) {
  const base: FideliOSConfig = {
    $meta: {
      version: 1,
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/fidelios-db",
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/fidelios-storage" },
      s3: {
        bucket: "fidelios",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/fidelios-secrets/master.key" },
    },
  };
