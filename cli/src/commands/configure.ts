...
function defaultConfig(): FideliOSConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" fidelios configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `fidelios onboard` first.");
    p.outro("");
    return;
  }

  let config: FideliOSConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because FIDELIOS_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
