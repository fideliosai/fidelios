...
export const DEFAULT_WORKTREE_HOME = "~/.fidelios-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".fidelios");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: FideliOSConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "fidelios",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    FIDELIOS_HOME: paths.homeDir,
    FIDELIOS_INSTANCE_ID: paths.instanceId,
    FIDELIOS_CONFIG: paths.configPath,
    FIDELIOS_CONTEXT: paths.contextPath,
    FIDELIOS_IN_WORKTREE: "true",
    ...(branding?.name ? { FIDELIOS_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { FIDELIOS_WORKTREE_COLOR: branding.color } : {}),
  };
}
