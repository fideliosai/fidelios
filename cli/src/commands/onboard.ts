...
type OnboardDefaults = Pick<FideliOSConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "FIDELIOS_PUBLIC_URL",
  "DATABASE_URL",
  "FIDELIOS_DB_BACKUP_ENABLED",
  "FIDELIOS_DB_BACKUP_INTERVAL_MINUTES",
  "FIDELIOS_DB_BACKUP_RETENTION_DAYS",
  "FIDELIOS_DB_BACKUP_DIR",
  "FIDELIOS_DEPLOYMENT_MODE",
  "FIDELIOS_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "FIDELIOS_ALLOWED_HOSTNAMES",
  "FIDELIOS_AUTH_BASE_URL_MODE",
  "FIDELIOS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "FIDELIOS_STORAGE_PROVIDER",
  "FIDELIOS_STORAGE_LOCAL_DIR",
  "FIDELIOS_STORAGE_S3_BUCKET",
  "FIDELIOS_STORAGE_S3_REGION",
  "FIDELIOS_STORAGE_S3_ENDPOINT",
  "FIDELIOS_STORAGE_S3_PREFIX",
  "FIDELIOS_STORAGE_S3_FORCE_PATH_STYLE",
  "FIDELIOS_SECRETS_PROVIDER",
  "FIDELIOS_SECRETS_STRICT_MODE",
  "FIDELIOS_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const publicUrl =
    process.env.FIDELIOS_PUBLIC_URL?.trim() ||
    process.env.FIDELIOS_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.FIDELIOS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.FIDELIOS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.FIDELIOS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.FIDELIOS_ALLOWED_HOSTNAMES
    ? process.env.FIDELIOS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.FIDELIOS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.FIDELIOS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.FIDELIOS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.FIDELIOS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.FIDELIOS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.FIDELIOS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.FIDELIOS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.FIDELIOS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.FIDELIOS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.FIDELIOS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.FIDELIOS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.FIDELIOS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.FIDELIOS_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.FIDELIOS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "FIDELIOS_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<FideliOSConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printFideliOSCliBanner();
  p.intro(pc.bgCyan(pc.black(" fidelios onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveFideliOSInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: FideliOSConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        ...
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `fidelios doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("FIDELIOS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.FIDELIOS_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("FIDELIOS_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("FIDELIOS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: FideliOSConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: FIDELIOS_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("fidelios run")}`,
      `Reconfigure later: ${pc.cyan("fidelios configure")}`,
      `Diagnose setup: ${pc.cyan("fidelios doctor")}`,
    ].join("\n"),
    "Next commands",
  );
