...
import type { FideliOSConfig } from "../config/schema.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.FIDELIOS_CONFIG;
...
const WORKTREE_NAME_PREFIX = "fidelios-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.FIDELIOS_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.FIDELIOS_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromFideliOSConfig(config: FideliOSConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromFideliOSConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.fidelios${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".fidelios", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".fidelios", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.fidelios"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: FideliOSConfig, envEntries: Record<string, string>, portOverride?: number): string {
...
export function copySeededSecretsKey(input: {
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.FIDELIOS_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.FIDELIOS_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.FIDELIOS_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.FIDELIOS_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: FideliOSConfig;
  targetConfig: FideliOSConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveFideliOSEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readFideliOSEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://fidelios:fidelios@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "fidelios");
    const targetConnectionString = `postgres://fidelios:fidelios@127.0.0.1:${targetHandle.port}/fidelios`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readFideliOSEnvEntries(resolveFideliOSEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.FIDELIOS_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.FIDELIOS_AGENT_JWT_SECRET);
  mergeFideliOSEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { FIDELIOS_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadFideliOSEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run FideliOS inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" fidelios worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printFideliOSCliBanner();
  p.intro(pc.bgCyan(pc.black(" fidelios worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasFideliOSConfig: existsSync(path.resolve(worktreePath, ".fidelios", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printFideliOSCliBanner();
  p.intro(pc.bgCyan(pc.black(" fidelios worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveFideliOSEnvFile(configPath);
  const envEntries = readFideliOSEnvEntries(envPath);
  const out = {
    FIDELIOS_CONFIG: configPath,
    ...(envEntries.FIDELIOS_HOME ? { FIDELIOS_HOME: envEntries.FIDELIOS_HOME } : {}),
    ...(envEntries.FIDELIOS_INSTANCE_ID ? { FIDELIOS_INSTANCE_ID: envEntries.FIDELIOS_INSTANCE_ID } : {}),
    ...(envEntries.FIDELIOS_CONTEXT ? { FIDELIOS_CONTEXT: envEntries.FIDELIOS_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasFideliOSConfig)
      .map((choice) => path.resolve(choice.worktree, ".fidelios", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readFideliOSEnvEntries(resolveFideliOSEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start FideliOS once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasFideliOSConfig ? "fidelios" : "no-fidelios-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".fidelios", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".fidelios", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .fidelios/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasFideliOSConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a FideliOS worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasFideliOSConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No FideliOS worktrees were found. Run `fidelios worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target FideliOS configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local FideliOS instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated FideliOS instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with fidelios- if needed (created at ~/fidelios-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: FIDELIOS_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: FIDELIOS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source FIDELIOS_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local FideliOS instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like FideliOS worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with fidelios- if needed")
