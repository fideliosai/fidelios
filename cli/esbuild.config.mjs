/**
 * esbuild configuration for building the fidelios CLI for npm.
 *
 * Bundles all workspace packages (@fideliosai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @fideliosai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@fideliosai/")) {
      externals.add(name);
    }
  }
