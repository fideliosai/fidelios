...
export interface DiscoveredPlugin {
  ...
  manifest: FideliOSPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.fidelios/plugins/ local directory
  | "npm"               // npm packages matching fidelios-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: FideliOSPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the fidelios-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "fidelios-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: FideliOSPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover FideliOS plugins installed as npm packages in the current
   * Node.js environment matching the "fidelios-plugin-*" naming convention.
   *
   ...
   * the "fideliosPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a FideliOS plugin.
   * Throws if the package is a FideliOS plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<FideliOSPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: FideliOSPluginManifestV1;
    newManifest: FideliOSPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
export function isPluginPackageName(name: string): boolean {
  ...
  // Also accept scoped packages like @acme/plugin-linear or @fideliosai/plugin-*
  if (name.includes("/")) {
...
function resolveManifestPath(
  ...
  if (
    ...
    const manifestRelPath = (fideliosPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: FideliOSPluginManifestV1): string | undefined {
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: FideliOSPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as FideliOSPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a FideliOS plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<FideliOSPluginManifestV1> {
    let raw: unknown;
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<FideliOSPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @fideliosai/shared exports). Run those workers through
