 ...
 */
import { pluginManifestV1Schema } from "@fideliosai/shared";
import type { FideliOSPluginManifestV1 } from "@fideliosai/shared";
import { PLUGIN_API_VERSION } from "@fideliosai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): FideliOSPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as FideliOSPluginManifestV1,
        };
      }
