...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".fidelios.yaml" || normalized === ".fidelios.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const fidelios = isPlainRecord(metadata?.fidelios) ? metadata?.fidelios as Record<string, unknown> : null;
...
function findFideliOSExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".fidelios.yaml"] === "string") return ".fidelios.yaml";
  if (typeof files[".fidelios.yml"] === "string") return ".fidelios.yml";
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "fidelios_bundled") return true;
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      requireBoardApprovalForNewAgents:
        typeof fideliosCompany.requireBoardApprovalForNewAgents === "boolean"
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const fideliosAgentsOut: Record<string, Record<string, unknown>> = {};
    const fideliosProjectsOut: Record<string, Record<string, unknown>> = {};
    const fideliosTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const fideliosRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readFideliOSSkillSyncPreference(
    ...
    const fideliosAgents = Object.fromEntries(
      Object.entries(fideliosAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const fideliosProjects = Object.fromEntries(
      Object.entries(fideliosProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const fideliosTasks = Object.fromEntries(
      Object.entries(fideliosTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const fideliosRoutines = Object.fromEntries(
      Object.entries(fideliosRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeFideliOSSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
