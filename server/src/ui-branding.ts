const FAVICON_BLOCK_START = "<!-- FIDELIOS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- FIDELIOS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- FIDELIOS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- FIDELIOS_RUNTIME_BRANDING_END -->";
...
function createFaviconDataUrl(background: string, foreground: string): string {
  const svg = [
    ...
    `<circle cx="12" cy="12" r="8" stroke="${foreground}" stroke-width="2" fill="none"/>`,
    `<path stroke="${foreground}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none" d="M4.05 13c-1.7 1.8-2.5 3.5-1.8 4.5 1.1 1.9 6.4 1 11.8-2s8.9-7.1 7.7-9c-.6-1-2.4-1.2-4.7-.7"/>`,
    "</svg>",
  ].join("");
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.FIDELIOS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.FIDELIOS_WORKTREE_NAME) ?? nonEmpty(env.FIDELIOS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.FIDELIOS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="fidelios-worktree-enabled" content="true" />',
    `<meta name="fidelios-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="fidelios-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="fidelios-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
