...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@fideliosai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  fideliosUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).fideliosUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.fideliosUpgradeContext = context;
