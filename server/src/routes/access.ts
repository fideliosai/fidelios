...
function resolveFideliOSSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const fideliosSkillNames = new Set<string>();
  if (fideliosSkillsDir) {
    try {
      for (const entry of fs.readdirSync(fideliosSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) fideliosSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.fideliosApiUrl)) {
    ...
    if (legacyFideliOSApiUrl) merged.fideliosApiUrl = legacyFideliOSApiUrl;
  }
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawFideliOSApiUrl =
    typeof defaults.fideliosApiUrl === "string"
  ...
  if (rawFideliOSApiUrl) {
    try {
      ...
      if (
        parsedFideliOSApiUrl.protocol !== "http:" &&
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your FideliOS host.",
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/fidelios-claimed-api-key.json and load FIDELIOS_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: fideliosApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@fidelios.local";
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.fideliosApiUrl &&
