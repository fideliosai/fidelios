...
import { execute } from "@fideliosai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects fidelios env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the fidelios heartbeat.");
      expect(promptArg).toContain("FideliOS runtime note:");
      ...
      expect(invocationPrompt).toContain("FideliOS API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { FIDELIOS_TEST_CAPTURE_PATH: capturePath },
        },
        context: {},
        authToken: "t",
        onLog: async () => {},
