...
describe("company portability", () => {
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/fidelios/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "fidelios",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/fidelios/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "fidelios",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized FideliOS extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "FideliOS"');
    ...
    expect(extension).not.toContain("fideliosSkillSync");
  ...
  it("exports default sidebar order into the FideliOS extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".fidelios.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("exports the company logo into images/ and references it from .fidelios.yaml", async () => {
    ...
    expect(exported.files[".fidelios.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            cleanupCommand: "rm -rf .fidelios-tmp",
            remoteProvider: null,
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported FideliOS",
      },
  ...
  it("reads env inputs back from .fidelios.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with FideliOS routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "fidelios-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported FideliOS" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported FideliOS"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .fidelios.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported FideliOS");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".fidelios.yaml"] = `${exported.files[".fidelios.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
