...
import { execute } from "@fideliosai/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects fidelios env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the fidelios heartbeat.");
      ...
      expect(capture.prompt).toContain("FideliOS runtime note:");
      expect(capture.prompt).toContain("FIDELIOS_API_KEY");
      expect(invocationPrompt).toContain("FideliOS runtime note:");
      expect(invocationPrompt).toContain("FIDELIOS_API_URL");
    } finally {
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const fideliosDir = await createSkillDir(runtimeSkillsRoot, "fidelios");
