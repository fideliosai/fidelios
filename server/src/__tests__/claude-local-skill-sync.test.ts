...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "fideliosai/fidelios/fidelios-create-agent";
  ...
  it("defaults to mounting all built-in FideliOS skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === fideliosKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat FideliOS skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("fidelios-claude-user-skills-");
    ...
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      ...
      readOnly: true,
      detail: "Installed outside FideliOS management in the Claude skills home.",
    }));
  });
});
