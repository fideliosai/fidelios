...
import type { agents } from "@fideliosai/db";
import { sessionCodec as codexSessionCodec } from "@fideliosai/adapter-codex-local/server";
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[fidelios] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables FideliOS-managed rotation by default for codex and claude local", () => {
