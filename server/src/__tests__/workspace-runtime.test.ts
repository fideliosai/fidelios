...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "fidelios@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "FideliOS Test"]);
...
afterEach(async () => {
  ...
  delete process.env.FIDELIOS_WORKTREES_DIR;
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$FIDELIOS_WORKSPACE_BRANCH\" > .fidelios-provision-branch",
        "printf '%s\\n' \"$FIDELIOS_WORKSPACE_BASE_CWD\" > .fidelios-provision-base",
        "printf '%s\\n' \"$FIDELIOS_WORKSPACE_CREATED\" > .fidelios-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".fidelios-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".fidelios-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local FideliOS config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const fideliosHome = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(fideliosHome, "instances", instanceId);
    ...
    process.env.FIDELIOS_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/fidelios"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".fidelios", "config.json");
      const envPath = path.join(workspace.cwd, ".fidelios", ".env");
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-runtime-workspace-"));
  ...
  it("does not leak parent FideliOS instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "fideliosConfig: process.env.FIDELIOS_CONFIG ?? null,",
          "fideliosHome: process.env.FIDELIOS_HOME ?? null,",
          "fideliosInstanceId: process.env.FIDELIOS_INSTANCE_ID ?? null,",
    ...
    process.env.FIDELIOS_CONFIG = "/tmp/base-fidelios-config.json";
    process.env.FIDELIOS_HOME = "/tmp/base-fidelios-home";
    process.env.FIDELIOS_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/fidelios";
    ...
    expect(captured.fideliosConfig).toBeNull();
    expect(captured.fideliosHome).toBeNull();
    expect(captured.fideliosInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "fidelios-runtime-sibling-"));
