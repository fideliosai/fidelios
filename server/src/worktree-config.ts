...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by FideliOS worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.FIDELIOS_IN_WORKTREE !== "true") return null;

  const configPath = resolveFideliOSConfigPath(overrideConfigPath);
  const envPath = resolveFideliOSEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.FIDELIOS_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.FIDELIOS_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.FIDELIOS_HOME) ??
      nonEmpty(env.FIDELIOS_WORKTREES_DIR) ??
      "~/.fidelios-worktrees",
  );
...
function writeConfigFile(configPath: string, config: FideliOSConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".fidelios", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as FideliOSConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: FideliOSConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: FideliOSConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.FIDELIOS_HOME = context.homeDir;
  process.env.FIDELIOS_INSTANCE_ID = context.instanceId;
  process.env.FIDELIOS_CONFIG = context.configPath;
  process.env.FIDELIOS_CONTEXT = context.contextPath;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    FIDELIOS_HOME: context.homeDir,
    FIDELIOS_INSTANCE_ID: context.instanceId,
    FIDELIOS_CONFIG: context.configPath,
    FIDELIOS_CONTEXT: context.contextPath,
    FIDELIOS_IN_WORKTREE: "true",
    FIDELIOS_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: FideliOSConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as FideliOSConfig;
  } catch {
    return;
  }
