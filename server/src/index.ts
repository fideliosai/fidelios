...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.FIDELIOS_SECRETS_PROVIDER === undefined) {
    process.env.FIDELIOS_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.FIDELIOS_SECRETS_STRICT_MODE === undefined) {
    process.env.FIDELIOS_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.FIDELIOS_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.FIDELIOS_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.FIDELIOS_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.FIDELIOS_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set FIDELIOS_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@fidelios.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.FIDELIOS_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://fidelios:fidelios@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "fidelios");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://fidelios:fidelios@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "fidelios");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: fidelios");
    }
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.FIDELIOS_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or FIDELIOS_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.FIDELIOS_LISTEN_HOST = runtimeListenHost;
  process.env.FIDELIOS_LISTEN_PORT = String(listenPort);
  process.env.FIDELIOS_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.FIDELIOS_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.FIDELIOS_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "FideliOS server failed to start");
    process.exit(1);
  });
}
