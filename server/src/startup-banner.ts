...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.FIDELIOS_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.FIDELIOS_AGENT_JWT_SECRET === "string" ? parsed.FIDELIOS_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm fidelios onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveFideliOSEnvPath();
