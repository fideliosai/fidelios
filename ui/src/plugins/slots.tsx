 ...
 *
 * Plugin bundles are built with `external: ["@fideliosai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__fideliosPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__fideliosPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__fideliosPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__fideliosPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@fideliosai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@fideliosai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@fideliosai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@fideliosai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@fideliosai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__fideliosPluginBridge__) {
 ...
 * self-register via `window.fideliosPlugins.registerReactComponent()`.
 ...
 * of `@fideliosai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
