...
type FideliOSSprite = (typeof FIDELIOS_SPRITES)[number];
...
function spriteSize(sprite: FideliOSSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = FIDELIOS_SPRITES[(baseCol + baseRow) % FIDELIOS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = FIDELIOS_SPRITES[Math.floor(Math.random() * FIDELIOS_SPRITES.length)]!;
