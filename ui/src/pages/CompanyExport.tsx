...
/**
 * Filter .fidelios.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterFideliOSYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://fidelios.nl) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm fidelios company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [FideliOS](https://fidelios.nl) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [FideliOS](https://fidelios.nl) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .fidelios.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .fidelios.yaml
    const yamlPath = exportData.fideliosExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterFideliOSYaml(exportData.files[yamlPath], checkedFiles);
    }
