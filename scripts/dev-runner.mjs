...
const devServerStatusFilePath = path.join(repoRoot, ".fidelios", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".fidelios/dev-server-status.json",
]);
...
if (mode === "watch") {
  ...
  env.FIDELIOS_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.FIDELIOS_DEPLOYMENT_MODE = "authenticated";
  env.FIDELIOS_DEPLOYMENT_EXPOSURE = "private";
  env.FIDELIOS_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[fidelios] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[fidelios] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[fidelios] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.FIDELIOS_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[fidelios] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  ...
  if (result.code !== 0) {
    console.error("[fidelios] plugin sdk build failed");
