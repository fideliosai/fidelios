...
## Scope
This plan is now **gateway-only**. FideliOS supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses FideliOS skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a FideliOS task.
