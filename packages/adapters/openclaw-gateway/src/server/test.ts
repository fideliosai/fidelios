...
import { asString, parseObject } from "@fideliosai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the FideliOS server host.",
        });
      }
    } catch (err) {
      checks.push({
