...
const DEFAULT_CLIENT_VERSION = "fidelios";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "fidelios";
  if (input.strategy === "run") return `fidelios:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `fidelios:issue:${input.issueId}`;
  return fallback;
}
...
function resolveFideliOSApiUrlOverride(value: unknown): string | null {
...
function buildFideliOSEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const fideliosApiUrlOverride = resolveFideliOSApiUrlOverride(ctx.config.fideliosApiUrl);
  const fideliosEnv: Record<string, string> = {
    ...buildFideliOSEnv(ctx.agent),
    FIDELIOS_RUN_ID: ctx.runId,
  };

  if (fideliosApiUrlOverride) {
    fideliosEnv.FIDELIOS_API_URL = fideliosApiUrlOverride;
  }
  if (wakePayload.taskId) fideliosEnv.FIDELIOS_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) fideliosEnv.FIDELIOS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) fideliosEnv.FIDELIOS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) fideliosEnv.FIDELIOS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) fideliosEnv.FIDELIOS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    fideliosEnv.FIDELIOS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return fideliosEnv;
}

function buildWakeText(payload: WakePayload, fideliosEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/fidelios-claimed-api-key.json";
  const orderedKeys = [
    "FIDELIOS_RUN_ID",
    "FIDELIOS_AGENT_ID",
    "FIDELIOS_COMPANY_ID",
    "FIDELIOS_API_URL",
    "FIDELIOS_TASK_ID",
    "FIDELIOS_WAKE_REASON",
    "FIDELIOS_WAKE_COMMENT_ID",
    "FIDELIOS_APPROVAL_ID",
    "FIDELIOS_APPROVAL_STATUS",
    "FIDELIOS_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = fideliosEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = fideliosEnv.FIDELIOS_API_URL ?? "<set FIDELIOS_API_URL>";

  const lines = [
    "FideliOS wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `FIDELIOS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load FIDELIOS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $FIDELIOS_API_KEY on every API call.",
    "- Use X-FideliOS-Run-Id: $FIDELIOS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: FIDELIOS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$FIDELIOS_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$FIDELIOS_COMPANY_ID/issues?assigneeAgentId=$FIDELIOS_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardFideliOSPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  fideliosEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateFideliOS = parseObject(payloadTemplate.fidelios);
  const workspace = asRecord(ctx.context.fideliosWorkspace);
  const workspaces = Array.isArray(ctx.context.fideliosWorkspaces)
    ? ctx.context.fideliosWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.fideliosRuntimeServiceIntents)
    ? ctx.context.fideliosRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardFideliOS: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: fideliosEnv.FIDELIOS_API_URL ?? null,
  };

  if (workspace) {
    standardFideliOS.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardFideliOS.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardFideliOS.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "fidelios-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const fideliosEnv = buildFideliOSEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, fideliosEnv);
  ...
  const fideliosPayload = buildStandardFideliOSPayload(ctx, wakePayload, fideliosEnv, payloadTemplate);
