...
- a minimal React widget rendered in the FideliOS dashboard
...
```bash
pnpm --filter @fideliosai/plugin-hello-world-example build
pnpm fidelios plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm fidelios plugin uninstall fidelios.hello-world-example --force` then  
  `pnpm fidelios plugin install ./packages/plugins/examples/plugin-hello-world-example`.
