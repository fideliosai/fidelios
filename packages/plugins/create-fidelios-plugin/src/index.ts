...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".fidelios-sdk");
...
/**
 * Generate a complete FideliOS plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A FideliOS plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "fidelios-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["fidelios", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-fidelios-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <fidelios-sdk-path>]");
    process.exit(1);
  }
