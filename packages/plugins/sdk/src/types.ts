/**
 * Core types for the FideliOS plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@fideliosai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @fideliosai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of FideliOS object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the FideliOS UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit FideliOS domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core FideliOS domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the FideliOS secret provider
...
export interface PluginSecretsClient {
   ...
   * in the FideliOS secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@fideliosai/plugin-sdk/ui`
 */
