...
Agents need API keys to authenticate with FideliOS. The current approach
...
**Trust model:** The adapter process runs on the same machine as the FideliOS
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** FideliOS generates a token and passes it directly to the agent
...
`FIDELIOS_API_URL`, `FIDELIOS_AGENT_ID`, `FIDELIOS_COMPANY_ID`. We need to
add a `FIDELIOS_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a FideliOS CLI
...
```
fidelios auth login
# Opens browser -> user confirms -> token stored at ~/.fidelios/credentials
```
...
managed by the FideliOS server itself.
...
1. A company admin (user or agent) generates an **invite URL** from FideliOS.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The FideliOS SKILL.md (or a link to it)
   - What information FideliOS needs from the agent (e.g. webhook URL, adapter
...
5. FideliOS stores the pending registration.
...
7. On approval, FideliOS provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** FideliOS issues an API key (or JWT) upon approval, delivered
...
1. Generate a FideliOS invite link for the company.
...
4. A FideliOS company member approves the new agent.
5. FideliOS begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `FIDELIOS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `fidelios auth login` for developer-managed remote agents.                                      |
