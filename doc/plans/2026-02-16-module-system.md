...
FideliOS's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends FideliOS's API, UI, and data model | Yes |
...
modules/
  observability/
    fidelios.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@fideliosai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@fideliosai/core";
...
```
1. Scan modules/ directory for fidelios.module.json manifests
...
```jsonc
// fidelios.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@fideliosai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@fideliosai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:fidelios/mod-observability",
      "version": "0.1.0",
...
```bash
pnpm fidelios store list                    # browse available modules and templates
pnpm fidelios store install <module-id>     # install a module
pnpm fidelios store import <template-id>    # import a company template
pnpm fidelios store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one FideliOS instance | `server:started` |
...
4. **Module config** — `fidelios.config.json` with per-module config, env var interpolation
...
10. **`@fideliosai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
