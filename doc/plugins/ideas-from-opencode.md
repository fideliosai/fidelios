...
FideliOS V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a FideliOS-shaped design.

Assumption for this document: FideliOS is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class FideliOS objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into FideliOS.

The main conclusion is:

- FideliOS should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- FideliOS should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- FideliOS should use multiple extension classes instead of one generic plugin bag:
...
If FideliOS does this well, the examples you listed become straightforward:
...
Relevant FideliOS files reviewed for current extension seams:
...
It is too dangerous for FideliOS core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for FideliOS — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
FideliOS should absolutely do this.
...
FideliOS should copy this discipline.
...
FideliOS should adopt this style for plugin actions, automations, and UI schemas.
...
FideliOS can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for FideliOS too.

## What FideliOS Should Not Copy Directly
...
FideliOS is a control plane for an operator-managed instance with company objects.
...
FideliOS is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

FideliOS should not auto-load arbitrary code from a workspace repo like `.fidelios/plugins` or project directories.
...
For FideliOS, equivalent hooks into:
...
FideliOS should not allow plugins to silently replace:
...
For FideliOS it would be risky because it combines:
...
FideliOS should require an explicit operator install step.

## Why FideliOS Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | FideliOS |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted FideliOS instance |
...
That means FideliOS should borrow the good ideas from `opencode` but use a stricter architecture.

## FideliOS Already Has Useful Pre-Plugin Seams

FideliOS has several extension-like seams already:
...
This is good news.
FideliOS does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended FideliOS Plugin Model
...
FideliOS already has this pattern implicitly:
...
- `plugin` means integration code that talks to FideliOS through a typed plugin protocol and capability model
...
- invoke explicit FideliOS actions through the API
...
FideliOS should treat plugin installation as a global instance-level action.
...
FideliOS already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for FideliOS too.
...
Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how FideliOS achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the FideliOS server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. FideliOS's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For FideliOS
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit FideliOS entities
    });
...
Any FideliOS plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific FideliOS objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of FideliOS itself
...
If a concept is becoming part of FideliOS's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of FideliOS itself
...
### 3. Scope plugin data to FideliOS objects before adding custom schemas

A lot of plugin data naturally hangs off existing FideliOS objects:
...
If FideliOS eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving FideliOS.
- Board reviews the diff after a run without leaving FideliOS.
...
- linking FideliOS issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between FideliOS and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into FideliOS.
- FideliOS issue status changes push to Linear and Linear comments arrive back through webhooks.
...
- linking PRs to FideliOS issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with FideliOS task governance
...
- Plugin settings:
  ...
  - whether new FideliOS issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active FideliOS issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into FideliOS.
- GitHub webhooks update status/comment state in FideliOS.
- A PR is linked back to the FideliOS issue so the board can follow delivery status.
...
- creating FideliOS issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full FideliOS plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the FideliOS dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a FideliOS issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of FideliOS core; keep it as additive signal and issue creation
...
- Failed payment spikes or churn events can generate FideliOS issues for follow-up.
...
- Stripe data should stay additive to FideliOS core
...
FideliOS should not implement "an OpenCode-style generic in-process hook system."
FideliOS should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In FideliOS
