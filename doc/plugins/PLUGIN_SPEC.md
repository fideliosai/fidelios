# FideliOS Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for FideliOS's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main FideliOS app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary FideliOS HTTP APIs directly.
...
FideliOS plugin design is based on the following assumptions:

1. FideliOS is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core FideliOS business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by FideliOS core.
...
2. Let plugins add major capabilities without editing FideliOS core.
...
4. Depend on project-local plugin folders such as `.fidelios/plugins`.
...
The single FideliOS deployment an operator installs and controls.

### 5.2 Company

A first-class FideliOS business object inside the instance.
...
A trusted in-process extension loaded directly by FideliOS core.
...
An installable instance-wide extension package loaded through the FideliOS plugin runtime.
...
FideliOS has two extension classes.
...
FideliOS already has a concrete workspace model:
...
Plugins live under the FideliOS instance directory.

Suggested layout:

- `~/.fidelios/instances/default/plugins/package.json`
- `~/.fidelios/instances/default/plugins/node_modules/`
- `~/.fidelios/instances/default/plugins/.cache/`
- `~/.fidelios/instances/default/data/plugins/<plugin-id>/`
...
FideliOS should add CLI commands:

- `pnpm fidelios plugin list`
- `pnpm fidelios plugin install <package[@version]>`
- `pnpm fidelios plugin uninstall <plugin-id>`
- `pnpm fidelios plugin upgrade <plugin-id> [version]`
- `pnpm fidelios plugin doctor <plugin-id>`
...
Plugins may contribute tools that FideliOS agents can use during runs.
...
- FideliOS server starts one worker process per installed plugin
...
Receives one typed FideliOS domain event.
...
export function definePlugin(definition: PluginDefinition): FideliOSPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: FideliOSPluginManifestV1;
  config: {
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the FideliOS secret provider system rather than a plain text input
...
1. Core FideliOS data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing FideliOS objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual FideliOS product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if FideliOS later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing FideliOS secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm fidelios plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the FideliOS server. This is a normative requirement, not optional.
...
For developing a plugin against a running FideliOS instance:

- The operator installs the plugin from a local path: `pnpm fidelios plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-fidelios-plugin`) that scaffolds:

- `package.json` with correct `fideliosPlugin` keys
...
FideliOS should not implement a generic in-process hook bag modeled directly after local coding tools.

FideliOS should implement:
...
That is the complete target design for the FideliOS plugin system.
