...
**Ticket references are links (required):** If you mention another issue identifier such as `TST-224`, `ZED-24`, or any `{PREFIX}-{NUMBER}` ticket id inside a comment body or issue description, wrap it in a Markdown link:

- `[TST-224](/PAP/issues/TST-224)`
- `[ZED-24](/ZED/issues/ZED-24)`

Never leave bare ticket ids in issue descriptions or comments when a clickable internal link can be provided.

**Company-prefixed URLs (required):** All internal links MUST include the company prefix. Derive the prefix from any issue identifier you have (e.g., `TST-315` → prefix is `PAP`). Use this prefix in all UI links:

- Issues: `/<prefix>/issues/<issue-identifier>` (e.g., `/PAP/issues/TST-224`)
...
Do NOT use unprefixed paths like `/issues/TST-123` or `/agents/cto` — always include the company prefix.
...
- Source issue: [TST-142](/PAP/issues/TST-142)
- Depends on: [TST-224](/PAP/issues/TST-224)
```
